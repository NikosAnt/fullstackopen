...
describe('Note app', () => {
  ...
  test('front page can be opened', async ({ page }) => {
    const notesHeading = page.getByText('Notes')
    const footerText = page.getByText(
      'Note app, Department of Computer Science, Freelance 2025'
    )
    await expect(notesHeading).toBeVisible({ timeout: 10000 })
    await expect(footerText).toBeVisible({ timeout: 10000 })
  })

  test('user can log in', async ({ page }) => {
    await loginWith(page, 'mluukkai', 'Sala1nen')
